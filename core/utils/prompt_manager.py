...
class PromptManager:
    ...
    def _load_base_template(self):
        """加载基础提示词模板"""
        try:
            inline_template = str(self.config.get("prompt_template_content") or "").strip()
            if inline_template:
                self.base_prompt_template = inline_template
                self.logger.bind(tag=TAG).debug("从配置加载内联基础提示词模板")
                return
